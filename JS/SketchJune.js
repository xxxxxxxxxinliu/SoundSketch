...
const minFreqHz = 500;//C3
const maxFreqHz = 3200;//C7
...
*/
	...
	for(let i = 0; i< moveNum ; i++){
		dialogs[i]= new Dialog(Number(x[i])*windowWidth, Number(y[i])*windowHeight, text[i], font[i],color[i],Number(size[i]),Number(speed[i]),Number(time[i]),false);
	...
	for(let z = 0; z< popNum ; z++){
		...
		dialogp[z]= new DialogP(Number(px[z])*windowWidth, Number(py[z])*windowHeight, ptext[z], pfont,pcolor[z],Number(psize[z]),pborntime[z],pdeadtime[z],false);
		//print(pfont);
		print(psize)
	}
		...
		for (let i = maxFreqHz; i >= minFreqHz; i--) {
			...
			else if (frameCount %10 >=3 && frameCount %10 <5)
			{
				...
				if(intensity>150){
					...
					//red
					historygram.stroke(67,43,86,50);
